...
lines = nvidia_smi.stdout.decode('utf-8').split('\n')[:-1]
...
def colorize(_lines):
    for i in range(len(_lines)):
        line = _lines[i]
        m = re.match(
            r"\| ..%\s+[0-9]{2,3}C.*\s([0-9]+)MiB\s+\/\s+([0-9]+)MiB.*\s([0-9]+)%", line)
        if m is not None:
...
if len(pid) == 0 or pid[0] == 'running':
    processes = []
else:
    ps_format = "pid,user,%cpu,%mem,etime,command"
    processes = subprocess.run(
        ["ps", "-o", ps_format, "-p", ",".join(pid)], stdout=subprocess.PIPE)
    processes = processes.stdout.decode().split("\n")
...
for line in processes:
    ...
    for idx in range(len(pid)):
        ...
        mem[idx] = parts[3]
        time[idx] = parts[
            4] if not "-" in parts[4] else parts[4].split("-")[0] + " days"
        command[idx] = parts[5][0:100]

format = ("|  %3s %5s %8s   %8s %5s %5s %9s  %-" +
          str(command_length) + "." + str(command_length) + "s  |")
